...
public class HUDResponder: StatusResponder {
    ...
    public func changeStatus(
        ...
        if let last = lastAnimator, last.isRunning {
            ...
            last.finishAnimation(at: .current) // 不能用 end,因为用回调时机不好控制，用 end 会完成走完流程，有可能会在新方法调用后把视图清理了
        }
        // 准备执行动画，设置相关视图的起始状态
        ...
        view.snp.makeConstraints {
            ...
            case .top:
                ...
                if #available(iOS 11.0, *) {
                    ...
                    $0.top.equalTo(chrysan.snp.top).offset(layout.offset.y)
                }
            case .bottom:
                $0.centerX.equalToSuperview().offset(layout.offset.x)
                if #available(iOS 11.0, *) {
                    ...
                    $0.bottom.equalTo(chrysan.snp.bottom).inset(layout.offset.y)
                }
            }
            ...
            if #available(iOS 11.0, *) {
                ...
                $0.left
                    .greaterThanOrEqualTo(chrysan.snp.left)
                    .inset(layout.padding.left)
                $0.right
                    .lessThanOrEqualTo(chrysan.snp.right)
                    .inset(layout.padding.right)
                $0.top
                    .greaterThanOrEqualTo(chrysan.snp.top)
                    .inset(layout.padding.top)
                $0.bottom
